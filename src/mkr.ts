...
export type ChairsRequest = {
  structureId: number,
  chairId: number    
}

export type TeacherScheduleRequest = ChairsRequest & {
  teacherId: number,
  startDate?: Date,
  endDate?: Date
}
...
export class MkrApi {
    ...
    private teacherSchedules: Cache<KeyValuePair[]> = new Cache({ defaultTtl: cacheTTL })
    ...
    async loadChairs(structureId: number): Promise<KeyValuePair[]> {
      ...
      const html = await this.sendForm(this.url, data);
    ...
    async loadTeacherSchedule(request: TeacherScheduleRequest): Promise<Event[]> {
      ...
      const html = await this.sendForm(this.url + '?type=1', data)  
      ...
      const events = eventsList.map((event: any) => {
        const type = lessonTypes.get(event.className) || event.className
    ...
    private async sendForm(url: string, data: any): Promise<string> {    
        ...
        if (response.status === 400) {
            this.csrfToken = extractCsrfToken(new JSDOM(html));
            console.log("Updated CSRF Token:", this.csrfToken);
